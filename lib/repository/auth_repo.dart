...
class AuthRepository {
  Future<Map<String, dynamic>?> sendOtp(
    ...
    final endpoint = ApiUrl.auth.sendOtp;
    ...
    final message = 'a_verification_code_sent_to_your_phone_number'.recast;
  ...
  Future<User?> verifyOtpCode(Map<String, dynamic> body) async {
    final endpoint = ApiUrl.auth.verifyOtp;
    final finalBody = await _deviceInfo(body);
    final apiResponse = await sl<ApiInterceptor>().postRequest(endpoint: endpoint, body: finalBody);
    if (apiResponse.status == 200) {
      final authApi = AuthApi.fromJson(apiResponse.response['data']);
  ...
  Future<AuthApi?> createAccount(Map<String, dynamic> body) async {
    final endpoint = ApiUrl.auth.createAccount;
    final finalBody = await _deviceInfo(body);
    final apiResponse = await sl<ApiInterceptor>().postRequest(endpoint: endpoint, body: finalBody);
    if (apiResponse.status == 200) {
      final authApi = AuthApi.fromJson(apiResponse.response['data']);
      ...
      final isUDisc = message != null && message == 'The udisc username has already been taken.';
      if (isUDisc) FlushPopup.onWarning(message: 'the_udisc_username_has_already_exists'.recast);
      final isPgda = message != null && message == 'The pdga number has already been taken.';
      if (isPgda) FlushPopup.onWarning(message: 'the_pdga_number_has_already_exists'.recast);
      final isEmailExist = message != null && message == 'The email has already been taken.';
  ...
  Future<AuthApi?> checkSocialLogin(Map<String, dynamic> body) async {
    final endpoint = ApiUrl.auth.checkSocialLogin;
    final finalBody = await _deviceInfo(body);
    final apiResponse = await sl<ApiInterceptor>().postRequest(endpoint: endpoint, body: finalBody);
    if (apiResponse.status != 200) return null;
    final authApi = AuthApi.fromJson(apiResponse.response['data']);
  ...
  Future<bool> checkUniqueUser(Map<String, dynamic> body) async {
    final endpoint = ApiUrl.auth.checkUnique;
    final apiResponse = await sl<ApiInterceptor>().postRequest(endpoint: endpoint, body: body);
    if (apiResponse.status == 200) return true;
    // final emailMessage = 'this_email_already_exist_please_try_with_another_email';
    // final phoneMessage = 'this_phone_number_already_exist_please_try_with_another_phone_number';
