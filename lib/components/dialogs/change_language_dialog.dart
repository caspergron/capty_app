...
Future<void> changeLanguageDialog({required Country country, Function(Language)? onProceed}) async {
  final context = navigatorKey.currentState!.context;
  // sl<AppAnalytics>().screenView('change-language-popup');
  await showGeneralDialog(
    context: context,
    barrierLabel: 'Change Language Dialog',
    transitionDuration: DIALOG_DURATION,
    transitionBuilder: sl<Transitions>().topToBottom,
    pageBuilder: (buildContext, anim1, anim2) => PopScopeNavigator(canPop: false, child: Align(child: _DialogView(country, onProceed))),
  );
}

class _DialogView extends StatelessWidget {
  final Country country;
  final Function(Language)? onProceed;
  const _DialogView(this.country, this.onProceed);
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            '${'do_you_want_to_change_the_language_to'.recast} ${country.language?.name ?? ''} ${'based_on_your_selected_country'.recast} ${country.name.toKey.isEmpty ? '' : '(${country.name})'}?',
            textAlign: TextAlign.center,
