...
Future<void> postFeedbackSheet({required Feature suggestion, required Function(String) onPost}) async {
  final context = navigatorKey.currentState!.context;
  final padding = MediaQuery.of(context).viewInsets;
  final child = _BottomSheetView(suggestion, onPost);
...
class _BottomSheetView extends StatefulWidget {
  final Feature suggestion;
  final Function(String) onPost;
  const _BottomSheetView(this.suggestion, this.onPost);
...
class _BottomSheetViewState extends State<_BottomSheetView> {
  ...
  @override
  void initState() {
    // sl<AppAnalytics>().screenView('post-feedback-sheet');
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Expanded(child: Text('post_your_feedback'.recast, style: TextStyles.text18_600.copyWith(color: lightBlue))),
          ...
          child: Column(
            children: [
              ...
              Align(
                ...
                child: ElevateButton(height: 40, radius: 06, padding: 20, label: 'post_feedback'.recast.toUpper, onTap: _onPost),
              ),
            ],
          ),
        ),
  ...
  InputDecoration get _decoration {
    return InputDecoration(
      ...
      hintText: 'write_your_feedback_here'.recast,
    );
  }

  void _onPost() {
    minimizeKeyboard();
    if (_feedback.text.isEmpty) return FlushPopup.onWarning(message: 'please_write_your_feedback'.recast);
