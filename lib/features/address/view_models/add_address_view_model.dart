...
class AddAddressViewModel with ChangeNotifier {
  ...
  Future<void> initViewModel(Address item) async {
    final google = sl<GoogleRepository>();
    if (item.id == null) {
      ...
      final coordinates = position.is_coordinate ? position : await google.coordinatesOfACountry(countryCode: country.code!);
      ...
      final coordinates = item.is_coordinate ? item.coordinates : await google.coordinatesOfACountry(countryCode: country.code!);
      if (coordinates?.is_coordinate == true) centerLocation = coordinates!;
      notifyListeners();
      await Future.delayed(const Duration(milliseconds: 300));
      if (coordinates?.is_coordinate == true) centerLocation = coordinates!;
  ...
  Future<void> fetchAddressPredictions(String input, Coordinates coordinates) async {
    suggestions.clear();
    final response = await sl<GoogleRepository>().fetchPredictions(input, coordinates, country.code!);
  ...
  Future<Map<String, dynamic>?> fetchAddressInfoByPlaceId(String input) async {
    final response = await sl<GoogleRepository>().addressInfoByPlaceId(input);
    suggestions.clear();
  ...
  Future<Map<String, dynamic>?> fetchAddressInfoByCoordinates(Coordinates coordinates) async {
    final response = await sl<GoogleRepository>().addressInfoByCoordinates(coordinates);
    suggestions.clear();
    if (response != null) addressInfo = response;
    notifyListeners();
    return response;
  ...
  Future<void> onCreateOrUpdateAddress(Address item) async {
    loader = true;
    notifyListeners();
    final isAddress = item.id == null;
    final context = navigatorKey.currentState!.context;
    final coordinates = addressInfo?['coordinates'] as Coordinates;
    final addressLabel = item.label.toKey == 'home'.toKey ? 'home' : 'other';
    final body = {
    ...
    final response = isAddress ? await repo.createAddress(body) : await repo.updateAddress(body, item);
    loader = false;
    if (response == null) return notifyListeners();
    notifyListeners();
    final addressModel = Provider.of<SellerSettingsViewModel>(context, listen: false);
