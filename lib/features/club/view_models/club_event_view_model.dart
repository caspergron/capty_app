...
class ClubEventViewModel with ChangeNotifier {
  ...
  Future<void> _fetchClubComments() async {
    var response = await sl<ClubRepository>().fetchEventComments(event);
  ...
  Future<void> onJoinEvent() async {
    loader.common = true;
    notifyListeners();
    var body = {'club_event_id': event.id, 'club_id': event.club?.id};
  ...
  Future<void> onPostComment(String comment, ScrollController scrollControl) async {
    ...
    var messageInfos = _commentInfo(dateMillisecond, comment);
